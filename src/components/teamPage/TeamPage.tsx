            ...
            <div className='flex flex-wrap gap-10'>
                {groupedSchedules.length > 0 ? ( // 그룹화된 스케줄이 있는 경우
                    groupedSchedules.map((group, index) => (
                        <div key={index} className='h-56 max-h-56 w-56 rounded-lg text-sm shadow-lg flex flex-col text-center justify-center items-center'>
                            <p className='sticky top-0 bg-mainColor text-white w-full text-center mb-2 p-2 rounded-t-lg'>{group.user_name} 님의 업무 계획입니다.</p> {/* 다른 유저의 이름 표시 */}
                            <div className='overflow-y-auto flex-grow p-2'>
                                ...
                                <ul className='list-disc'>
                                    {group.schedules.map((schedule, idx) => (
                                        <li key={idx} className='text-left mb-2 border-b'>{schedule.title}</li>
                                    ))}
                                </ul>
                            </div>
                        </div>
