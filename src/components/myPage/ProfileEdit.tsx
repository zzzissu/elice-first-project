...
const ProfileEdit = () => {
    ...
    const handleUpdatePhoneNumber = async () => {
        const token = localStorage.getItem('token');
        try {
            const res = await fetch("http://localhost:4000/api/profile/phone", {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`,
        ...
        <div>
            <div className='EditSiteTotal flex flex-col justify-center mt-10'>
                ...
                <button
                    ...
                    className='flex text-gray-400 w-20 h-auto justify-end '>
                    회원탈퇴
            ...
            <DeleteModal
                isOpen={isDeleteModalOpen} onClose={handleDeleteModalClose} onDelete={userDelete}/>
        </div>
    )
}
